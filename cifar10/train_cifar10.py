...
from lib.data_utils import processing_img, convert_img_back, convert_img, Batch, shuffle, iter_data, ImgRescale, \
    ImgBatchRescale, OneHot
...
def load_data():
    ...
    ys = []
    for j in range(5):
        d = unpickle('E:\\Code\\EvolutionaryGAN\\cifar-10-batches-py\\data_batch_' + str(j + 1))
        x = d['data']
        y = d['labels']
        xs.append(x)
        ys.append(y)

    d = unpickle('E:\\Code\\EvolutionaryGAN\\cifar-10-batches-py\\test_batch')
    xs.append(d['data'])
    ys.append(d['labels'])
...
# easier to read.
def create_G(loss_type=None, discriminator=None, lr=0.0002, b1=0.5, ng=64):
    global generator_loss
    noise = T.matrix('noise')
    # noise=T.row("noise")
    generator = models_uncond.build_generator_toy(noise, ng=ng)
...
def main():
    ...
    if not os.path.exists("E:\\Code\\EvolutionaryGAN\\cifar-10-batches-py"):
        print(
    ...
    discriminator = models_uncond.build_discriminator_toy(nd=nd, GP_norm=GP_norm)
    ...
    for epoch in range(niter):
        ...
        for xmb in iter_data(X_train, size=batchSize * kD):  # For measure fitness score
            ...
            if epoch + n_updates == 0:
                for can_i in range(0, ncandi):
                    train_g_, gen_fn_, generator_ = create_G(
                        ...
                        discriminator=discriminator, lr=lr, b1=b1, ng=ng)
